...
export const useAttractionStore = defineStore("attractionStore", () => {
  ...
  const getMyPlanList = async (userId = undefined) => {
    try {
      for (let idx = 0; idx < sidoCodes.length; idx++) {
        let response;
        if (userId) {
          response = await axios.get(`/plan/list/${userId}/${sidoCodes[idx]}`);
        } else {
          response = await axios.get(`/plan/mylist/${sidoCodes[idx]}`, {
  ...
  const setMyPlan = (sidoCode) => {
    selectMyPlans.value = myPlanList.value.find(
